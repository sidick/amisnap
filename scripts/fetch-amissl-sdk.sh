...
# package, for Phase 3 item 4 (src/amiga/tls.c, soft-loaded AmiSSL). Lives
# under scripts/, not tests/, because it's a real *build*-time dependency
# now, not just test tooling: `make m68k` calls this itself (see the
# Makefile's own m68k target) to get AmiSSL's headers + libamisslstubs.a
# to compile/link tls.c against (house rule 6: verify against real
# headers, not memory) -- amisslmaster.library itself stays a soft,
# runtime-only dependency (proposal.md's "absent = plaintext fallback"
# policy), but the *headers* a build needs are a separate concern from
# whether the *library* is installed on the machine that later runs the
# binary. The OS3 runtime package's amisslmaster.library/amissl_v*.
# library are for actually exercising AmiSSL on-target under Copperline
# (not yet wired into a test harness -- see implementation-plan.md).
#
# Adapted from sibling AmiAuth's tests/copperline/fetch-amissl-sdk.sh
# (same pinned release, same verification approach -- no reason to
# duplicate divergent logic for the same public archive).
#
# Fetches two archives from the same pinned release:
#   - AmiSSL-<ver>-SDK.lha  -- headers + libamisslstubs.a, for linking
#     against m68k-amigaos-gcc.
#   - AmiSSL-<ver>-OS3.lha  -- the actual runtime amisslmaster.library
#     and the CPU-specific amissl_v*.library, which must be staged onto
#     a Copperline boot volume (LIBS:/LIBS:AmiSSL/) for OpenLibrary() to
#     succeed. The SDK alone does not include these.
#
# Idempotent: downloads + extracts only when the pinned-version
# directory is missing. Verifies SHA-256 on both archives. On success,
# prints two lines to stdout: the SDK's Developer dir, then the OS3
# package's AmiSSL dir.
#
# Usage:
...
# Pinned SHA-256s for both archives -- same release AmiAuth already
# verified. If AMISSL_VERSION is changed, matching hashes must be
# supplied via AMISSL_SDK_SHA256/AMISSL_OS3_SHA256 -- refuse to
