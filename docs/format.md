...
| 0x8016 | OBJCOMP     | u8: 0 = raw objects, 1 = framed objects (see Content objects). Fixed at init; writers MUST always emit it, and readers treat an absent tag as 0 (headers written before OBJCOMP existed are raw by construction). Which algorithm a framed repository's writer *prefers* (LZ4 vs deflate) is client configuration, not repository state — each object's frame names its own algorithm, so writers with different preferences can share a repository. |
