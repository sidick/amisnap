...
| 0x8016 | OBJCOMP     | u8: 0 = raw objects, 1 = framed objects (see Content objects). Fixed at init; writers MUST always emit it, and readers treat an absent tag as 0 (headers written before OBJCOMP existed are raw by construction). Each object's frame names its own algorithm, so writers with different preferences can share a repository. |
| 0x0017 | COMP_PREF   | u8, present iff OBJCOMP = 1: the frame algorithm (1 = LZ4, 2 = zlib) the user chose at init, recorded so a writer with no per-run override knows what to compress new objects with. Informational and non-critical: a *reader* never needs it (frames are self-describing), and a writer MAY override it per run without updating it. |
