#!/usr/bin/env python3
"""docs2guide.py - build AmiSnap.guide from the userdocs/ pages.
...
Adapted directly from sibling AmiAuth's tool of the same name (the
Markdown-to-AmigaGuide conversion logic is generic, not AmiAuth-specific)
-- only PAGES and version() below are AmiSnap's own. Deliberately a
pragmatic subset-of-Markdown converter, tuned to how the pages are
actually written (see PAGES): headings, bold, inline code, fenced code
blocks, pipe tables, lists, blockquotes, [md](links) - links to sibling
...
# Site nav order; (file-slug, node title). MAIN is index (the landing page).
PAGES = [
    ('index', 'AmiSnap'),
...
def version(root):
    """VERSION.REVISION from version.mk, and the $VER cookie's own build
    date from src/cli/main.c -- the two places AmiSnap's real version
    already lives (see CLAUDE.md "House conventions"), not a third,
    independently-maintained copy of either."""
    ver = '0.1'
    vdate = ''
    try:
        mk = open(os.path.join(root, 'version.mk')).read()
        major = re.search(r'VERSION\s*:?=\s*(\S+)', mk).group(1)
        minor = re.search(r'REVISION\s*:?=\s*(\S+)', mk).group(1)
        ver = '%s.%s' % (major, minor)
    except Exception:
        pass
    try:
        # verstring is built from macro-expanded VERSION/REVISION
        # (XSTR(VERSION) "." XSTR(REVISION)), so there's no literal
        # "AmiSnap 0.1" substring in the source to match against --
        # only the trailing build-date parenthetical is literal text.
        src = open(os.path.join(root, 'src/cli/main.c')).read()
        vdate = re.search(r'\(\d\d\.\d\d\.\d\d\d\d\)', src).group(0)[1:-1]
    except Exception:
        pass
    return ver, vdate
...
def main():
    ...
    head = [
        '@database AmiSnap',
        '@$VER: AmiSnap.guide %s (%s)' % (ver, vdate),
        '@author Simon Dick',
        '@(c) BSD 2-Clause; https://github.com/sidick/amisnap',
        '@width %d' % (WIDTH + 2),
        '',
    ]
    body = []
    for n, (slug, title) in enumerate(PAGES):
        ...
        body.append('')
        if slug == 'index':
            body.append('@{b}AmiSnap %s@{ub} - versioned network backup '
                        'with snapshots for classic AmigaOS' % ver)
            body.append('')
